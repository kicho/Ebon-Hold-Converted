    ...
    void UpdateAI(const uint32 diff) 
    {
        if (uiSpeech_counter >= 1 && uiSpeech_counter <= 6)
            if (uiSpeech_timer < diff)
            {
                ...
                Unit* pPlayer = (Creature*)Unit::GetUnit(*m_creature, uiPlayerGUID);
...
struct MANGOS_DLL_DECL mob_dark_rider_of_acherusAI : public ScriptedAI
{
    ...
    {
        if (Unit* pPlayer = (Creature*)Unit::GetUnit(*m_creature, uiPlayerGUID))
         ...
         if (HasEscortState(STATE_ESCORT_PAUSED))
        {
            if (m_uiWave_Timer < uiDiff)
            {
                switch(m_uiWave)
                {
                    ...
                    case 4:
                    {
                        Creature* pTemp = (Creature*)Unit::GetUnit(*m_creature, m_uiValrothGUID);
...
struct MANGOS_DLL_DECL npc_highlord_darion_mograineAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i)
    {
        switch(i)
        {
            ...
            case 3:
            {
                Unit* pTirion = (Creature*)Unit::GetUnit((*m_creature), uiTirionGUID);
            ...
            if (uiPhase_timer < diff)
            {
                ...
                switch(uiStep)
                {
                    ...
                    case 4: // summon ghoul
                        ...
                        {
                            Creature* pTemp = m_creature->SummonCreature(NPC_ACHERUS_GHOUL, (m_creature->GetPositionX()-20)+rand()%40, (m_creature->GetPositionY()-20)+rand()%40, m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000);
                    ...
                    case 5: // summon abomination
                        ...
                        {
                            Creature* pTemp = m_creature->SummonCreature(NPC_RAMPAGING_ABOMINATION, (m_creature->GetPositionX()-20)+rand()%40, (m_creature->GetPositionY()-20)+rand()%40, m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000);
                            pTemp->RemoveMonsterMoveFlag(MONSTER_MOVE_WALK);
                            pTemp->setFaction(2084);
                    ...
                    case 6: // summon warrior
                        ...
                        {
                            Creature* pTemp = m_creature->SummonCreature(NPC_WARRIOR_OF_THE_FROZEN_WASTES, (m_creature->GetPositionX()-20)+rand()%40, (m_creature->GetPositionY()-20)+rand()%40, m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000);
                            pTemp->RemoveMonsterMoveFlag(MONSTER_MOVE_WALK);
                            pTemp->setFaction(2084);
                    ...
                    case 7: // summon warrior
                        ...
                        {
                            Creature* pTemp = m_creature->SummonCreature(NPC_FLESH_BEHEMOTH, (m_creature->GetPositionX()-20)+rand()%40, (m_creature->GetPositionY()-20)+rand()%40, m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000);
                            pTemp->RemoveMonsterMoveFlag(MONSTER_MOVE_WALK);
                            pTemp->setFaction(2084);
                    ...
                    case 29: // lich king spawns
                        if (Creature* pTemp = m_creature->SummonCreature(NPC_THE_LICH_KING, LightofDawnLoc[26].x, LightofDawnLoc[26].y, LightofDawnLoc[26].z, LightofDawnLoc[26].o, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                        {
                            ...
                            if (Unit* pAlex = (Creature*)Unit::GetUnit((*m_creature), uiAlexandrosGUID))
                    ...
                    case 30:
                        if (Creature* pTemp = (Creature*)Unit::GetUnit((*m_creature), uiAlexandrosGUID)) // just hide him
                        {
...
//   doesn't make it looks well, so workarounds, Darion charges, looks better
                    ...
                    case 41: // Lich king - Apocalypse
                        if (Creature* pTemp = (Creature*)Unit::GetUnit((*m_creature), uiLichKingGUID))
                        {
                            ...
                            if (Unit* pTirion = (Creature*)Unit::GetUnit((*m_creature), uiTirionGUID))
                            {
